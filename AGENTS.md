...
- **缓存**：Service 方法上的 `@Cacheable`/`@CacheEvict` 需成对维护；`@CachePut` 只能用于返回实体本身的方法——历史上曾因 `@CachePut` 缓存了 boolean 返回值导致 `ClassCastException`（已修复，勿回退）。注意注解驱逐发生在事务提交前：外层事务较长时（含 REQUIRES_NEW 写入）并发读会在窗口内把旧值回填缓存，写入方法需经提交后驱逐兜底（先例：`service/impl/TransactionAfterCommit`）。另注意**同类内部调用（this.xxx）不走代理**，目标方法上的 `@Cacheable`/`@CacheEvict`/`@Transactional` 一律不生效——需要注解语义必须跨 bean 调用或手动操作 CacheManager（先例：LRCLIB 拉取编排独立为 `LyricFetchServiceImpl`，勿把编排逻辑并回数据存取类）。
